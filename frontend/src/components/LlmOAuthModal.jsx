 ...
 * Purpose: Pick provider, optionally enter OAuth app credentials, open provider login popup.
 */
 ...
 * @param {Array<{ id: string, label: string, hint?: string, ready?: boolean, needsAppCredentials?: boolean, redirectUri?: string }>} props.providers
...
export function LlmOAuthModal({ open, onClose, onConnected, providers, redirectUri, llmBaseUrl, llmModel }) {
  ...
  /**
   * Opens provider OAuth in a popup and waits for postMessage from callback page.
   */
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col gap-3 overflow-y-auto rounded-2xl border border-teal-100 bg-white p-4 shadow-xl">
            ...
            {list.map((p) => (
              ...
              >
                <span className="flex items-center gap-2 font-medium">
                  ...
                  {p.label}
                  {p.ready ? (
                    <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-xs text-emerald-800">ready</span>
                  ) : null}
                </span>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          >
            {busy ? "Waiting for sign-in…" : "Sign in (opens popup)"}
          </button>
        </div>
      </div>
    </div>
