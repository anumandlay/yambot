...
/**
 * Returns the Codex model id for OpenAI OAuth — rejects stale MiniMax/API-key model names.
 * @param {string} [storedModel]
 * @returns {string}
 */
export function resolveOpenAiOAuthModel(storedModel) {
  const raw = String(storedModel || "").trim();
  if (!raw) return OPENAI_CODEX_DEFAULT_MODEL;
  const lower = raw.toLowerCase();
  if (lower === "minimax" || lower.startsWith("minimax-") || lower.includes("minimax")) {
