...
export const LLM_OAUTH_PROVIDER_CATALOG = [
  {
    ...
    hint: "Sign in with your ChatGPT account — no API key. Uses your ChatGPT subscription via Codex backend.",
  },
];
...
export async function saveLlmOAuthTokens(user, providerId, tokens) {
  ...
  if (providerId === "openai") {
    ...
    // Why: always switch model on connect — leftover MiniMax-M2.7 breaks Codex API calls.
