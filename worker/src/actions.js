...
- message_agent: { "type":"message_agent", "to":"Exact Peer Name"|["B","C"], "mode":"task|question|approval|handoff|event", "content":"...", "wait": true|false|"soft", "soft_wait_minutes": 3 } — or fanout:[{ "to":"B", "content":"..." }, { "to":"C", "content":"..." }]. Fan-out queues peers in parallel (max 5). wait:true blocks until all finish; wait:false / "soft" keep working (PEER RESULT per peer). Prefer wait:false or "soft" when you have remaining work. Modes: task/question/approval/handoff/event. event defaults wait:false. Max hop depth 2 (A→B→C). Managers may only message managedAgents when that list is set. When the goal is “have peer open a site / do work and report back”, do not navigate that URL yourself.
- skills_list: { "type":"skills_list" } — list available production skills (name, slug, description). Use when no ACTIVE SKILL fits.
- skill_view: { "type":"skill_view", "slug":"vughy-register" } or { "type":"skill_view", "id":"…" } — load the full playbook for this run (Hermes-style progressive load).
- memory: { "type":"memory", "action":"add|replace|remove", "target":"user|memory", "content":"...", "old_text":"..." } — durable curated facts. DEFAULT target memory = this agent's MEMORY.md. When the human says “remember …” in this chat, use target memory. target user = account-wide USER.md identity/prefs only (or when they explicitly say user/account memory). Hard char caps — replace/remove when full. Writes apply next run; frozen prompt blocks do not change mid-run.
