...
- message_agent: { "type":"message_agent", "to":"Exact Peer Name", "mode":"task|question|approval|handoff|event", "content":"...", "wait": true|false|"soft", "soft_wait_minutes": 3 } — wait:true blocks; wait:false keeps working (PEER RESULT later); wait:"soft" works for soft_wait_minutes then pauses if peer still running. Prefer wait:false or "soft" when you have remaining work. Modes: task/question/approval/handoff/event. event defaults wait:false. Max hop depth 2 (A→B→C). Managers may only message managedAgents when that list is set. When the goal is “have peer open a site / do work and report back”, do not navigate that URL yourself.
