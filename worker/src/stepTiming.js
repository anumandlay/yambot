 ...
 * Inputs: env YAMBOT_* overrides + YAMBOT_FAST_MODE; Downstream: worker agent loop, semanticWait.
 */

import { getFastModeProfile, isFastMode } from "./fastMode.js";
...
export const stepTiming = {
  ...
  postActionSettleMs: Math.max(
    200,
    Number(process.env.YAMBOT_STEP_SETTLE_MS) || (fast ? 400 : 1000)
  ),
  /** Short settle after type/fill_form/select in a multi-action burst. */
  postFillSettleMs: Math.max(
    40,
    Number(process.env.YAMBOT_FILL_SETTLE_MS) || (fast ? 120 : 280)
  ),
  /** Longer settle during error-recovery retries. */
  recoverySettleMs: Math.max(
    400,
    Number(process.env.YAMBOT_RECOVERY_SETTLE_MS) || (fast ? 1200 : 2200)
  ),
  /** How long interactive count must stay unchanged to count as stable. */
  domStableMs: Math.max(60, Number(process.env.YAMBOT_DOM_STABLE_MS) || (fast ? 100 : 180)),
  /** Pause before retrying after an LLM HTTP failure. */
  llmRetryMs: Math.max(200, Number(process.env.YAMBOT_LLM_RETRY_MS) || (fast ? 400 : 800)),
  /** Pause before retrying after invalid model JSON. */
  parseRetryMs: Math.max(200, Number(process.env.YAMBOT_PARSE_RETRY_MS) || (fast ? 300 : 500)),
  ...
  defaultWaitActionMs: Math.max(
    100,
    Number(process.env.YAMBOT_DEFAULT_WAIT_MS) || (fast ? 400 : 750)
  ),
  /** Cap LLM multi-action batches so one turn cannot run forever. */
  maxActionsPerTurn: profile.maxActionsPerTurn,
};
